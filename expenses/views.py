...
def username_view(request):
    ...
    if request.method == "POST":
        ...
        if username:
            ...
            profile = Profile.objects.create(name=username)

            request.session["profile_id"] = str(profile.id)

            return redirect("dashboard")

    return render(request, "expenses/username.html")
...
def dashboard(request):
    ...
    scoped_expenses = Expense.objects.filter(
        profile=profile,
    ...
    total_spending = scoped_expenses.aggregate(
        total=Sum("amount")
    )["total"] or Decimal("0")
    ...
    monthly_data_qs = (
        Expense.objects.filter(profile=profile)
